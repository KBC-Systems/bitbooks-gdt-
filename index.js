					...
					.auto({
						...
						scheme : [
								...
								function(cb, scope) {
									try {
										...
										scope
												.logger("Failed to load blockchain.json");
									}

									var fields = [], aliasedFields = [], types = {}, selector = {};
									...
									for (i = 0; i < db.length; i++) {
										for (n = 0; n < db[i].tableFields.length; n++) {
											__field = db[i].alias + "."
													+ db[i].tableFields[n].name;
											;
											__alias = db[i].alias + "_"
													+ db[i].tableFields[n].name;
										...
										selector[db[i].table] = extend(db[i], {
																	...
																	.forEach(
																			function(
																				...
																				if (typeof (modules[namespace][moduleName][eventName]) == "function") {
																					modules[namespace][moduleName][eventName]
																							.apply(
																									modules[namespace][moduleName][eventName],
																									args);
																				}
						...
						ready : [
								...
								function(cb, scope) {
									...
									scope.logger("Dapp loaded process pid "
											+ process.pid)
									cb();
								} ]
					});
